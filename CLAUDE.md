...
7. **오늘 날짜가 키에 들어가면 TTL 을 준다.** 과거 날짜만 불변으로 캐시한다. 오늘 것은 아직
   확정되지 않았으므로(장중 시세, 그날의 부분 봉) 한 번 담아 두면 낡은 값을 계속 내놓는다.
   "행이 있다"는 것은 "그 값이 맞다"는 뜻이 아니다.

8. **오늘을 기계에게 묻지 않는다.** `date.today()`·`datetime.now()` 대신
   `app.clock.today_kst()`·`now_kst()` 를 쓴다. 개발 PC 는 KST 고 배포 서버는 UTC 라,
   기계 시간대를 따르면 한국시간 자정~아침 9시에 서버만 하루 뒤처진다. 회귀 테스트가
   `date.today()` 의 재등장을 막고 있다(`tests/test_clock.py`).
